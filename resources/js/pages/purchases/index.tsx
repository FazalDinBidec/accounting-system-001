...
import { dateInputValue, formatMoney } from '@/pages/purchases/types';
import type { PaginatedPurchases, PurchaseOrder } from '@/pages/purchases/types';

export default function PurchasesIndex({ purchases }: { purchases: PaginatedPurchases }) {
    ...
    return (
        <>
                ...
                <Card className="overflow-hidden py-0">
                    ...
                    <CardContent className="pb-0">
                        {purchases.data.length === 0 ? (
                            <p className="p-4 text-center text-sm text-muted-foreground">No purchases yet.</p>
                        ) : (
                            <>
                                <Table>
                                    <TableHeader>
                                    ...
                                    <TableBody>
                                        {purchases.data.map((purchase) => (
                                            <TableRow key={purchase.id}>
                                                ...
                                                <TableCell>{purchase.party?.name ?? '—'}</TableCell>
                                                <TableCell>{dateInputValue(purchase.date)}</TableCell>
                                                ...
                                                <TableCell className="text-center">
                                                    <div className="flex flex-wrap items-center justify-center gap-2">
                                                        <Button
                                                            ...
                                                            asChild
                                                        >
                                                            <Link href={PurchaseOrderController.edit.url(purchase)}>
                                                                <Pencil />
            ...
            <DeleteDialog
                ...
                action={deletePurchase ? PurchaseOrderController.destroy.form(deletePurchase) : undefined}
            />
        </>
    );
}
