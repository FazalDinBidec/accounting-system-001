...
export default function VoucherForm({
    ...
    useEffect(() => {
        ...
        const url = VoucherController.partyBalance.url(Number(partyId), {
            query:
                voucher === undefined ? {} : { exclude_voucher_id: voucher.id },
        });
        ...
        void fetch(url, {
            ...
            .catch(() => {
                if (!cancelled) {
                    setBalance((current) =>
                        current?.partyId === partyId ? null : current,
                    );
                }
            });
    ...
    const totalAmount = data.lines.reduce(
        (sum, line) => sum + toMoneyNumber(line.amount),
        0,
    );
